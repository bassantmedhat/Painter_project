...
void PaintArea::drawStraightLine(const QPoint startPoint, const QPoint endPoint)
{
    ...
    myEndPoint = endPoint;
    item = new line(myStartPoint, myEndPoint);
    sortVector.push_back(item);
    update();
}
...
void PaintArea::drawRectangle(const QPoint startPoint, const QPoint endPoint)
{
    ...
    rectlength = endPoint.y() - startPoint.y();
    rectwidth = endPoint.x() - startPoint.x();
    lastPoint = endPoint;

    item = new class Rectangle(rectlength, rectwidth);
    sortVector.push_back(item);
    update();
}
...
{
    QRectF area(startPoint.x(), startPoint.x(), endPoint.x() - startPoint.x(), endPoint.x() - startPoint.x());
    QPainter painter(&image);
    if (!_right)
    {
        painter.setBrush(myFillColor);
    ...
    myStartPoint = startPoint;
    myEndPoint = endPoint;
    item = new circle(myStartPoint, myEndPoint);
    sortVector.push_back(item);
    update();
}
...
QVector<Shapes *> PaintArea::getVect()
{
